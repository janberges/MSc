% !TEX root = ../slides.tex
%
\tikzsetnextfilename{dyson-3}
%
\begin{tikzpicture}[scale=0.7]
    \useasboundingbox (0, -0.7) rectangle (11, 1.4);

    \foreach \r in {(4, 0), (8, 0), (2, 0.7), (4, 0.7), (8, 0.7)}
        \draw [forward] \r -- +(1, 0);

    \foreach \r in {(0, 0.7), (6, 0.7)}
        \draw [forward, double] \r -- +(1, 0);

    \foreach \r in {(0, 0.0), (6, 0.0)}
        \draw [ inward, double] \r -- +(1, 0);

    \draw [ outward, double] (10, 0.7) -- +(1, 0);
    \draw [backward, double] (10, 0.0) -- +(1, 0);

    \draw [forward, double] (5, 0) arc (180:360:0.5);
    \draw [ inward, double] (9, 0) arc (180:360:0.5);
    \draw [ inward, double] (9, 0.7) arc (180:0:0.5);
    \draw [forward, double] (5, 0.7) arc (180:0:0.5);

    \foreach \x in {5, 9} {
        \foreach \y in {0.0, 0.7} {
            \draw [phonon, double] (\x, \y) -- +(1, 0);
            }
        }

    \foreach \x in {5, 6, 9, 10}
        \foreach \y in {0, 0.7}
            \fill (\x, \y) circle (0.075);

    \foreach \r in {(1.5, 0.7), (1.5, 0)} \node at \r {$=$};

    \foreach \r in {(3.5, 0.7), (7.5, 0.7), (7.5, 0)}
        \node at \r {$+$};

    \useasboundingbox ([yshift=1mm] current bounding box.north east);
\end{tikzpicture}%
