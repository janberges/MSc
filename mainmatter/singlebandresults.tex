...
\subsection{Convergence of the self-energy with the cutoff frequency}
...
\subsection{Convergence of the critical temperature with the cutoff frequency}
...
namely \name{McMillan}'s equation for the critical temperature. It was derived
starting from the cDOS \name{Eliashberg} theory, so that the latter should
...
pseudo-potentials. (\name{McMillan} calculated $\lambda$ for different $T \sub
...
known that \name{McMillan}'s formula intrinsically underestimates the critical
...
immanent in \name{McMillan}'s formula is perfectly confirmed. It may be
explained by the fact that $\omega \sub E$ enters the \name{Eliashberg}
equations only through $\lambda(n)$ or the phononic \name{Green} function, the
\q{broadening} of which is determined by $\frac T {\omega \sub E}$.
