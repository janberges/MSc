...
The basic idea of the local approximation is to reduce the dependence on wave numbers to a mere energy-dependence. Essentially,
%
\begin{equation*}
...
equation of $\Delta_i(\I \omega)$ nor must it be solved self-consistently. For $N \rightarrow \infty$ can further evaluate
%
...
\begin{equation}
   \begin{split}
      ...
      [\Lambda_{i j}^+(n, m) - \delta_{n m} D_{i j}^N(n) - 2 \mu^*_{i j}(N)],
      \\
      D_{i j}^N(n) &=
...
Assuming that the matrices $\vec \lambda$ and $\vec \mu^*(N)$ describing the
intra- and inter-band electron-phonon coupling strengths and rescaled
...
\end{equation*}
%
and thus, applying Eq.~\ref{eigenvalues direct product}, the largest eigenvalue
of $\vec K$ to be the product of the largest eigenvalues of $\vec Q$ and $\vec
...
\lambda$ and $\vec \mu^*(N)$, then the further are given by the greatest
eigenvalues of the latter.

If $\vec \lambda$ is not proportional to $\vec \mu^*(N)$ there are still
innumerable sets of effective scalar parameters which yield the same critical
temperature, but the presented rule will no longer work.
...
\end{align*}
%
Again, $T \sub c$ may be defined via the largest eigenvalue of $\vec K$ being
unity. However, the mapping onto effective scalar coupling strengths is
complicated by the presence of the density of states which differs from band to
band.
...
\begin{align*}
    ...
    & \text{For each $\omega \in \mathds R$ of interest:} \\
    & \qquad \Sigma(\omega) \leftarrow 1. \\[10pt]
    & \qquad \text{For $n = 1 \dots N - 1$ reversed:} \\
    & \qquad \qquad \Sigma(\omega) \leftarrow 1
        + \frac{c_n \, (\omega - \I \omega_{n - 1})}{\Sigma(\omega)}. \\[10pt]
