...
eigenvalues of the latter if the renormalization were unity.
...
%
\begin{equation*}
   K_{i j} = \lambda_{i j} + \hypo{\lambda_{i j}(1)}{\approx \lambda_{i j}}
   - \delta_{i j} \sum_j \lambda_{i j} - 2 \mu^*_{i j}
\end{equation*}
%
For a two-band system with $\mu^*_{i j} = 0$, which is only assumed for
simplicity, one has
%
\begin{equation*}
   \vec K = \begin{bmatrix}
...
\begin{equation*}
   \lambda = \frac 1 2 \bigg[
      \lambda_{1 1} - \lambda_{1 2} + \lambda_{2 2} - \lambda_{2 1}
      + \sqrt{
         (\lambda_{1 1} - \lambda_{1 2} - \lambda_{2 2} + \lambda_{2 1})^2
