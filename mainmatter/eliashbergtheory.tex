...
the \name{Eliashberg} equations on the imaginary frequency axis is derived,
...
%
\begin{equation*}
    \input{diagrams/nambu-dyson.tex}
\end{equation*}
%
...
\footnotetext{These derivations are guided by Section~3.2 of
Ref.~\barecite{Galasso05} and Section~II of Ref.~\barecite{MargineGiustino13}.}
%
...
\name{Migdal}'s theorem \cite{Migdal58}. Thus
%
\begin{multline*}
    \vec \Sigma_{\vec k} \super{ph.} (\I \omega_n)
...
equations} \cite{Eliashberg60} for the \name{Hubbard-Holstein} model:
%
\begin{subequations}
    \begin{align}
...
\name{Fermi}-surface average $\av{\av{\alpha^2 F}_{\epsilon_{\vec k} = \mu}
}_{\epsilon_{\vec k + \vec q} = \mu}$ \cite[Eqs.~3.23, 3.24]{AllenMitrovic82},
where
%
\begin{equation*}
    \av{f_{\vec k}}_{\epsilon_{\vec k} = \mu}
    = \frac
        {\sum_{\vec k} \delta(\epsilon_{\vec k} - \mu) \, f_{\vec k}}
        {\sum_{\vec k} \delta(\epsilon_{\vec k} - \mu)},
\end{equation*}
%
    ...
    \caption[CDOS approximation]{ %
        ...
        \begin{align*}
            ...
            \begin{tikzpicture}[baseline]
                \draw [draw=cyan, fill=cyan!50, rounded corners=3pt]
            ...
            \quad
            \begin{tikzpicture}[baseline]
                \draw [draw=orange, fill=orange!50, rounded corners=3pt]
                    (0, 0) rectangle (0.5, 0.2);
                \node [left=-2pt] at (0, 0.1) {(};
                \node [right=-2pt] at (0.5, 0.1) {)};
            \end{tikzpicture}.
...
over \name{Matsubara} frequencies, is performed within the CDOS approximation.
...
real-axis \name{Eliashberg} equations \cite[Eqs.~2, 3]{ScalapinoWadaSwihart65}:
%
...
In 1968, William L. \name{McMillan} establishes a formula to estimate the
...
Section~V of Ref.~\barecite{McMillanRowell69}.}
%
...
The idea is to find an analytic expression which roughly approximates $T \sub c$
...
course of which the maximum phonon frequency $\omega_0$ is replaced by the,
...
%
\footnote{According to \name{Schrieffer} \cite[187]{Schrieffer83} the formula
for $\mu^*$ was first given in 1959 by \name{Bogoliubov}, \name{Tolmachev} and
\name{Shirkov} \cite[83]{BogoliubovTolmachevShirkov59}. Nevertheless, the
quantity is sometimes referred to as the \emph{\name{Morel-Anderson}
pseudo-potential} with reference to Ref.~\barecite{MorelAnderson62} from 1962.}
%
in the preceding section requires a more detailed analysis of the \name{Coulomb}
interaction as occurring in the \name{Eliashberg} theory. Before that, however,
it is convenient to put the local and CDOS \name{Eliashberg} equations
...
solutions of the Eliashberg equations to be even functions of frequency.
Exploiting this symmetry it is possible to fold the negative half of the
\name{Matsubara} sum onto the positive one, just like it was done with the
...
consider the \name{Eliashberg} equation for $\phi \sub{el.}$,
%
...
\end{equation*}
%
or equivalently, introducing a \emph{rescaled \name{Coulomb} pseudo-potential}
$\mu^*(N)$,
%
\begin{equation*}
    \phi \sub{el.} =
...
rescaling the \name{Coulomb} interaction. For a computational solution for
...
via the \name{Stirling} formula \cite[Appendix~A]{AllenMitrovic82}.
Consequently,
%
\begin{align*}
...
\name{Coulomb} part at a lower frequency $\omega_N$, above which $\phi \sub{ph.}
...
of \name{McMillan's} equation. Combination of the results from
...
Analogously, for constant band densities of states one finds \cite[Eqs.~1,
2]{NicolCarbotte05}
%
...
band-structure, e.g. the first \name{Brillouin} zone, into subdomains for which
...
real-axis equations used by \name{McMillan}, the CDOS \name{Eliashberg}
...
Before the analysis can go more into detail, it is necessary to introduce the
...
\subsection{Effective scalar coupling strengths}
...
\name{McMillan}'s equation. It shall be noted that for all non-scalar couplings
...
\name{Coulomb} pseudo-potentials are proportional, i.e. $\mu^*_{i j}(N) = c \,
...
\lambda$ and $\vec \mu^*(N)$, then the former would be given by the greatest
...
\subsubsection{Cutoff-independent}
...
involve sums over already discrete \name{Matsubara} frequencies rather than
