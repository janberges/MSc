...
\end{equation*}
%
where the electronic part within the HF approximation according to
Eq.~\ref{HF approximation} reads
%
\begin{equation*}
...
\end{equation*}
%
and the corresponding phononic part
%
\begin{multline*}
    \vec \Sigma_{\vec k} \super{ph.} (\I \omega_n)
...
\begin{multline*}
    ...
    \\
    \dots \lambda_{\vec k - \vec q}(\I \omega_n - \I \omega_m) \{
        - \I \omega_m Z_{\vec q}(\I \omega_m) \vec \sigma_0
        + \phi_{\vec q}(\I \omega_m) \vec \sigma_1
        - [\epsilon_{\vec q} + \chi_{\vec q}(\I \omega_m)] \vec \sigma_3 \}
        - \mu \phi_{\vec q}(\I \omega_m) \vec \sigma_1 \big \},
\end{multline*}
%
...
\begin{align*}
    ...
    \\
    \lambda(\I \Omega_n) &= \int \from{-\infty} \till \infty \D \omega \,
        \frac{2 \omega \, \alpha^2 F(\omega)}{\Omega_n^2 + \omega}
...
diverges from $-\chi(\I \omega_m)$. Therefore, it can be a acceptable to
approximate $N(\epsilon)$ by a constant $N(0)$ so that the integration can be
...
\end{equation*}
%
where $\mathcal P$ denotes the \textsc{Cauchy} principal value, this yields
%
\begin{equation*}
    \vec \Sigma_{\vec k} (\I \omega_n)
    = \pi T \sum_m \frac{ \lambda(\I \omega_n - \I \omega_m) \{
        - \I \omega_m Z(\I \omega_m) \vec \sigma_0
        + \phi(\I \omega_m) \vec \sigma_1 \}
        - \mu \phi(\I \omega_m) \vec \sigma_1 }
        { \Xi(\I \omega_m) }
\end{equation*}
%
with $\Xi(\I \omega_n) = \sqrt{[\omega_n Z(\I \omega_n)]^2 + \phi^2(\I
\omega_n)}$ and thus the \emph{constant-DOS \name{Eliashberg} equations}
%
\begin{subequations}
    \begin{align}
        Z(\I \omega_n) &= 1 + \frac{\pi T}{\omega_n} \sum_m
        \frac{\omega_m Z(\I \omega_m)}{\Xi(\I \omega_m)}
        \lambda(\I \omega_n - \I \omega_m),
        \\
        \phi(\I \omega_n) &= \pi T \sum_m
        \frac{\phi(\I \omega_m)}{\Xi(\I \omega_m)}
        [\lambda(\I \omega_n - \I \omega_m) - \mu].
    \end{align}
\end{subequations}
