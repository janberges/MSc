...
of the functions $\omega^2$, $Z(\omega)$ and $\phi(\omega)$. They all have in
...
\omega_0)$, which will be derived in Eq.~\ref{normal-state renormalization},
whereas for the case of infinite frequency is trivial. Thus
%
\begin{equation*}
    Z(0) = 1 + \lambda, \quad Z(\infty) = 1.
\end{equation*}

It is now required that $\Delta(0) \equiv \Delta_0$ and $\Delta(\infty) \equiv
...
\begin{equation*}
    ...
    \quad \text{with} \quad
    \mu^* = \frac{\mu \sub C}{1 + \mu \sub C \ln(E \sub B / \omega_0)},
\end{equation*}
%
where the \emph{\name{Coulomb} pseudo-potential} $\mu^*$ has been identified.
...
\begin{align*}
    ...
    \Lambda^-(n, m),
    \\
    \Delta(\I \omega_n) &= \frac{\pi T}{Z(\I \omega_n)} \sum_{m = 0}^\infty
    \frac{\Delta(\I \omega_m)}{\sqrt{\omega_m^2 + \Delta^2(\I \omega_m)}}
    [\Lambda^+(n, m) - 2 \mu \sub C].
\end{align*}
%
...
rapidly with the magnitude of $\nu_n$. The \textsc{Coulomb} part, however, does
not depend on frequency and thus couples all terms equally strong. As a
consequence, the partial sum does not converge with increasing cutoff. The
following procedure%
%
...
\begin{equation} \label{Coulomb part of Eliashberg equation}
    \phi \sub{el.} = -\mu \sub C \pi T \sum_m^{\mathclap{|\omega_m| < \omega_u}}
    \frac
        {\phi(\I \omega_m)}
...
\begin{equation*}
    \Big[
        1 + \mu \sub C \pi T
        \sum_m^{\mathclap{\omega_l \leq |\omega_m| < \omega_u}} |\omega_m|^{-1}
    \Big]
    \phi \sub{el.} = -\mu \sub C \pi T \sum_m^{\mathclap{|\omega_m| < \omega_l}}
    \frac
        {\phi(\I \omega_m)}
        {\sqrt{[\omega_m Z(\I \omega_m)]^2 + \phi^2(\I \omega_m)}}
\end{equation*}
...
\begin{equation*}
    ...
    \frac
        ...
        {\sqrt{[\omega_m Z(\I \omega_m)]^2 + \phi^2(\I \omega_m)}}
    \quad \text{with} \quad
    \mu^* = \frac{\mu \sub C}{1 + \mu \sub C \pi T
...
%
\begin{equation*}
    \mu^* = \frac{\mu \sub C}{1 + \mu \sub C \ln(\omega_u / \omega_l)}.
\end{equation*}

\section{Chemical potential}
\label{chemical potential}

If the chemical potential in the above equations is assumed to be constant, the
particle number is not necessarily conserved. Following
...
\end{equation*}
%
where terms with $\I \omega_n Z_{\vec k}(\I \omega_n)$ have cancelled. Using
...
%
In order to conserve the particle number, the chemical potential $\mu$ has to be
determined self-consistently alongside the self-energy components.
...
finite set of \name{Matsubara} frequencies rather than analytical expressions,
