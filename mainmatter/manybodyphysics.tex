...
%
\begin{equation} \label{time-dependent ladder operator}
    \begin{aligned}
...
It is now shown how the full \name{Green} function is calculated. The idea is to
build it iteratively from what is already known, namely the \name{Green}
functions of the free particles and the coupling strengths of the interaction.
...
\begin{align*}
    G(\tau) &= -\av{\op T \, \op A(\tau) \, \op B} =
    -\Theta(\tau) \av{\op A(\tau) \, \op B}
    \pm \Theta(-\tau) \av{\op B(-\tau) \, \op A}
    \\
    &= \frac 1 {\av{S(\beta)}_0} \left \{
    \begin{aligned}
        - & \langle \op S(\beta) &&
        \op S^{-1}(\tau) && \op A[\tau] && \op S(\tau) &&
        \op B \rangle_0 && \text{for $0 < \tau < \beta$,} \\
        \pm & \langle \op S(\beta) &&
        \op S^{-1}(-\tau) && \op B[-\tau] && \op S(-\tau) &&
        \op A \rangle_0 && \text{for $-\beta < \tau < 0$.}
    \end{aligned}
    \right .
\end{align*}
%
...
%
\begin{equation*}
    G(\tau) = -\frac{\av{\op T \, \op A[\tau] \, \op S(\beta) \, \op B}_0}
    {\av{\op S(\beta)}_0}.
\end{equation*}

Using the above formula the \name{Matsubara-Green} function may be calculated
with an arbitrary order of accuracy by truncating the Taylor series in
Eq.~\ref{Matsubara-Dyson series} after the corresponding number of terms. In
doing so, expectation values of products of a growing number of creation and
annihilation operators will emerge. Fortunately, these can be factorized into
\name{Green} functions of free particles with the help of the \name{Wick}
theorem.

\subsection{\name{Wick} theorem}

\begin{equation*}
    [\op A_1, \prod_{i = 2}^{2 n} \op A_i]_\pm =
    \sum_{i = 2}^{2 n} (\mp 1)^i [\op A_1, \op A_i]_\pm
    \smash[b]{\prod_{\substack{j = 2 \\ j \neq i}}^n \op A_j},
\end{equation*}
%
e.g. in the case of six operators,
%
\begin{align*}
