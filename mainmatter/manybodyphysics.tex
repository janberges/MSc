...
particles is discussed. Subsequently, the perturbation series and \name{Wick}'s
theorem for non-zero temperatures are derived, which prepare the ground for
\name{Feynman}'s diagrammatical perturbation theory. For some model interactions
...
\end{equation}
%
The \textsc{Sokhotski–Plemelj} theorem leads to a useful expression for the
spectral function:
%
...
Eqs.~\ref{G_ret(omega)} one finds that $G \sub{ret.} (\omega)$ can be obtained
from $G(\I \omega)$ through \emph{analytic continuation} $\I \omega \rightarrow
\omega + \I 0^+$, since
%
...
which makes their solution a non-trivial task. In practice of often applies
