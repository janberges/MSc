...
theories of Tomonaga, Schwinger, and Feynman} \cite{Dyson49a} together with an
...
argument. This concerns e.g. mutual \name{Fourier} transforms as well as
...
expectation values. Let the \name{Hamilton} operator of the system be $\op H =
...
As opposed to this, in the \emph{\name{Heisenberg} picture} only the observables
depend on time. In this case differentiation using the product rule yields
\name{Heisenberg}'s equation of motion. Thus
%
\begin{equation*}
...
A useful compromise is provided by the \emph{\name{Dirac} picture}, also known
...
the \name{Hamilton} operator while the latter are governed by the interaction.
...
%
\footnote{See Eq.~32 in \name{Dyson}'s paper \cite{Dyson49a} for an analogous
formula, which he further discusses in Ref.~\barecite{Dyson49b}.}
%
...
The \emph{\name{Matsubara-Green} function} is preliminarily defined as
%
\begin{equation*}
...
consider a modified \name{Green} function which continues periodically beyond
this domain, namely
%
\begin{equation*}
...
\begin{equation} \label{Matsubara-Dyson series}
    \op S(\tau, \tau_0) = \sum_{n = 0}^\infty \frac{(-1)^n}{n!}
...
yields \emph{\name{Wick}'s theorem}%
%
\footnote{The theorem is named in honor of Ref.~\barecite{Wick50}. The
generalization to non-zero temperatures was first carried out by
\name{Matsubara} \cite{Matsubara55} and shown to be exact by \name{Thouless}
...
homogeneous electron gas and the simpler \name{Hubbard} model of interacting
electrons.
...
the unperturbed \name{Hamilton} operator with respect to which the averages are
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\linewidth/3}
        \centering
        \input{diagrams/fock-anomalous.tex}
        \caption{``\name{Fock}'', anomalous}
...
Substituting the corresponding non-interacting \name{Matsubara-Green} functions
and performing the summation to eliminate the \name{Kronecker} deltas, one
obtains
%
\begin{align*}
...
\end{align*}
%
The corresponding \emph{\name{Feynman} diagrams} are depicted in
...
\name{Matsubara} frequencies as per Eq.~\ref{Matsubara-Green function} yields
%
\begin{align*}
...
\end{align*}
%
has to be considered. With the help of \name{Wick}'s theorem this reduces to
%
\begingroup
    \def\minalignsep{0pt}
    \begin{align*}
...
\endgroup
%
Substituting \name{Green}'s functions, one finds two contributions which again
...
The interaction within an \emph{homogeneous electron gas} reads
\cite[165]{Czycholl08}
%
\begin{align*}
...
The first-order term of the \name{Feynman-Dyson} perturbation series for
...
order to ensure a causal time-ordering. The \name{Kronecker} delta is actually
superfluous for being contained in each term of $\av{\text{el.}}_0$ anyway. Once
...
instantaneous, are the leading \name{Hartree} and \name{Fock} self-energy
contributions. The former may be compensated exactly by a homogeneous positive
...
The \emph{\name{Hubbard} model} further restricts the \name{Coulomb} interaction
...
\name{Coulomb} repulsion $U$, the corresponding operator in real space reads
...
\begin{subequations} \label{Hedin equations}
    ...
    %
    \begin{equation}
        \input{diagrams/self-energy.tex},
    \end{equation}
    %
    ...
    %
    \begin{equation}
        \input{diagrams/phonon.tex}
        \qquad \text{with} \qquad
        \input{diagrams/polarization.tex},
    \end{equation}
    %
    ...
    %
    \begin{equation}
        \input{diagrams/vertex-exact.tex}.
    \end{equation}
\end{subequations}

Eqs.~\ref{Hedin equations}, known as \emph{\name{Hedin}'s equations}
...
A considerable simplification consists in neglecting all vertex corrections,
i.e. replacing $\Gamma$ with the bare vertex \cites[Eq.~A27,
A28]{Hedin65}[Eqs.~13.20, 13.21]{HedinLundqvist69}. The corresponding diagram
reads:
%
\begin{equation} \label{GW approximation}
    \input{diagrams/gw-self-energy.tex}
    \qquad \text{with} \qquad
    \input{diagrams/gw-polarization.tex}.
\end{equation}
%
...
\begin{equation} \label{HF approximation}
    \input{diagrams/hf-self-energy.tex}.
\end{equation}
%
